...
class DocumentParser:
    ...
    def parse_document(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            result = {
                ...
                'content': content[:500] + "..." if len(content) > 500 else content  # Сохраняем фрагмент для отчета
            }
            
            self.processed_docs.append(result)
            self.metrics[url] = quality_metrics
            return result
    ...
    def generate_report(self) -> Dict:
        ...
        successful_parses = [doc for doc in self.processed_docs if isinstance(doc, dict)]
        
        report = {
            'summary': {
                'total_urls': len(self.metrics),
                'successful_parses': len(successful_parses),
                'success_rate': len(successful_parses) / len(self.metrics) if self.metrics else 0,
        ...
        for doc in successful_parses:
            ...
            report['details'].append({
                ...
                'metrics': metrics,
                'content_sample': doc.get('content', '')[:200] + '...' if len(doc.get('content', '')) > 200 else doc.get('content', '')
            })
        
        report['summary']['document_types'] = dict(type_counts)
        
        if structure_scores:
            report['summary']['avg_structure_score'] = sum(structure_scores) / len(structure_scores)
        if uniqueness_scores:
            report['summary']['avg_uniqueness'] = sum(uniqueness_scores) / len(uniqueness_scores)
